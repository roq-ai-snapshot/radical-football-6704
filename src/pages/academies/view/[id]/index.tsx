...
import { deleteUserById } from 'apiSdk/users';
...
function AcademyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AcademyInterface>(
    ...
    () =>
      getAcademyById(id, {
        relations: ['user_academy_user_idTouser', 'player', 'user_user_academy_idToacademy'],
      }),
  );
  ...
  const userHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              ...
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      User User Academy Id Toacademies
                    </Text>
                    <NextLink passHref href={`/users/create?academy_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.user_user_academy_idToacademy?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/users/view/${record.id}`)} key={record.id}>
                            ...
                            <Td>
                              {hasAccess('user', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
